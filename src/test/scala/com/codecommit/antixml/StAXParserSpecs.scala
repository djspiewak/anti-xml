...
object StAXParserSpecs extends Specification {
  "StAXParser" should {
	...
	StAXParser.fromString("<a />").head must_== ElemStart("a", Map.empty)
      }
	...
	StAXParser.fromString("<a />").tail.head must_== ElemEnd(None, "a", None)
      }
      "generate namespace URIs" in {
	StAXParser.fromString("<a:a xmlns:a='http://example.com' />").head must beLike {
